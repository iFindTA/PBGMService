               ...
               unsigned char * prikey, unsigned long ulPrikeyLen)
{
    ...
    //     ECCSIGNATUREBLOB signature;
    //     memset(&signature, 0x00, sizeof(ECCSIGNATUREBLOB));
    unsigned char signatureR[100] = {0};//actually should be 64, in case of mem flew
    unsigned char signatureS[100] = {0};
    int rMax = 0, sMax = 0;
    ...
    ret = Mp_Int2Byte(signatureR+rMax, &ulTmp, &mp_r);
    CHECK_RET(ret);
    if (ulTmp > 32)
    {
        ret = ERR_UNKNOWN;
    ...
    ret = Mp_Int2Byte(signatureS+sMax, &ulTmp, &mp_s);
    CHECK_RET(ret);
    if (ulTmp > 32)
    {
        ret = ERR_UNKNOWN;
    ...
    memcpy(signedData, signatureR, 32);
    memcpy(signedData+32, signatureS, 32);
    * pulSigLen = 64;
                  ...
                  unsigned char * pri_dA, unsigned long ulPri_dALen)
{
    ...
    offsetX2 = (32 - tmpX2Len);
    offsetY2 = (32 - tmpY2Len);
    memset(ptmp, 0x00 , tmpX2Len * 3);
    memcpy(ptmp+offsetX2, tmpX2Buff, tmpX2Len+offsetX2);
    memcpy(ptmp+tmpX2Len+offsetX2+offsetY2, tmpY2Buff, tmpY2Len+offsetY2);
    ...
    // t= KDF(x2||y2, klen)
    ret = KDFwithSm3(pout, ptmp, tmpX2Len+tmpY2Len+offsetX2+offsetY2, C2_len);// t = pout
    CHECK_RET(ret);
    ...
    ptmp = new unsigned char[C2_len + tmpX2Len + tmpY2Len + offsetX2 + offsetY2 + 100];
    if (NULL == ptmp)
    {
        ret = ERR_MEM_ALLOC;
        goto END;
    }
    memset(ptmp, 0x00 , C2_len + offsetX2 + offsetY2 + tmpX2Len + tmpY2Len + 100);
    memcpy(ptmp+offsetX2, tmpX2Buff, tmpX2Len+offsetX2);
    memcpy(ptmp+tmpX2Len+offsetX2, pC2, C2_len);
    memcpy(ptmp+tmpX2Len+C2_len+offsetY2+offsetX2, tmpY2Buff, tmpY2Len+offsetY2);
    
    sm3(ptmp, tmpX2Len+C2_len+tmpY2Len+offsetX2+offsetY2, dgstC3);
                  ...
                  unsigned char * szPubkey_XY, unsigned long ul_PubkXY_len)
{
    ...
    do
    {
        ...
        // added 0x00 front
        int offsetX1 = 1;
        int nLengthX1 = mp_signed_bin_size(&mp_x1);
        for (int i = 0; i<(32 - (nLengthX1 - 1)); i++)
        {
            C1_buf[offsetX1] = 0x00;
            offsetX1 += 1;
        }
        
        Mp_Int2Byte(tmpBuff, &ulTmpBuffLen, &mp_x1);
        memcpy(C1_buf+offsetX1, tmpBuff, ulTmpBuffLen);
        
        // added 0x00 front
        int offsetY1 = 0;
        int nLengthY1 = mp_signed_bin_size(&mp_y1);
        for (int i = 0; i<(32 - (nLengthY1 - 1)); i++)
        {
            C1_buf[ulTmpBuffLen+offsetX1+offsetY1] = 0x00;
            offsetY1 += 1;
        }
        
        Mp_Int2Byte(tmpBuff, &ulTmpBuffLen2, &mp_y1);
        memcpy(C1_buf+offsetX1+ulTmpBuffLen+offsetY1, tmpBuff, ulTmpBuffLen2);
        C1_len = offsetX1+ ulTmpBuffLen + ulTmpBuffLen2+offsetY1;
        ...
        unsigned long offsetX2 = (32 - tmpX2Len);
        unsigned long offsetY2 = (32 - tmpY2Len);
        memset(ptmp, 0x00 , tmpX2Len * 3);
        memcpy(ptmp+offsetX2, tmpX2Buff, tmpX2Len+offsetX2);
        ...
        // t= KDF(x2||y2, klen)
        ret = KDFwithSm3(pout, ptmp, tmpX2Len+tmpY2Len+offsetX2+offsetY2, plainLen);// t = pout
        CHECK_RET(ret);
#ifdef _DEBUG
        MP_print_Space;
        printf("KDF t=");
    ...
    memcpy(ptmp+(32 - tmpX2Len), tmpX2Buff, tmpX2Len);
    memcpy(ptmp+tmpX2Len+(32 - tmpX2Len), plain, plainLen);
    memcpy(ptmp+tmpX2Len+(32 - tmpX2Len)+plainLen+(32 - tmpY2Len), tmpY2Buff, tmpY2Len);
    ...
    sm3(ptmp, tmpX2Len+(32 - tmpX2Len)+plainLen+tmpY2Len+(32 - tmpY2Len), C3_buf);
