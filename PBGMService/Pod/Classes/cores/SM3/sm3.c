...
{
    /// 个性化 by zdzhen:
    /// 这里原来是 unsigned long 类型，
    /// 由于和android编译平台不同所以不能使用long类型，否则会造成结果不一致
...
#define DROTL(x,n) (SHL((x),n) | ((x) >> ((32 - n)*-1)))
...
#define P1(x) ((x) ^  ROTL((x),15) ^ ROTL((x),23))
    ...
    for(j =16; j < 64; j++)
    {
        ...
        if ((32 - j) < 0)
        {
            ...
            unsigned int tM = ((T[j]) & 0xFFFFFFFF);
            unsigned int tL = (tM << (j - 32)) ;
            unsigned int tR = ((T[j]) >> (32 - (j - 32)));
            unsigned int t = (tL | tR);
    ...
    /// 个性化 by zdzhen:
    /// 因为使用了 unsigned int 可以计算出和 android 同样的结果
    /// 这里需要强制类型转换为 int 类型否则会造成结果不一致
    ctx->state[0] ^= (int)A;
    ctx->state[1] ^= (int)B;
    ctx->state[2] ^= (int)C;
    ctx->state[3] ^= (int)D;
